...
namespace TechTalk.SpecFlow.Infrastructure
{
    ...
    {
        bool Ready { get; }
        BindingMatch GetBestMatch(StepInstance stepInstance, out StepDefinitionAmbiguityReason ambiguityReason, out List<BindingMatch> candidatingMatches);
        BindingMatch Match(IStepDefinitionBinding stepDefinitionBinding, StepInstance stepInstance, bool useRegexMatching = true, bool useParamMatching = true, bool useScopeMatching = true);
    }
    ...
    public class StepDefinitionMatcher : IStepDefinitionMatchService
    {
        ...
        public BindingMatch GetBestMatch(StepInstance stepInstance, out StepDefinitionAmbiguityReason ambiguityReason, out List<BindingMatch> candidatingMatches)
        {
            candidatingMatches = GetCandidatingBindings(stepInstance, useParamMatching: true).ToList();
            ...
            if (candidatingMatches.Count > 1)
                ...
                ambiguityReason = OnNoMatch(stepInstance, out candidatingMatches);
        ...
        protected virtual StepDefinitionAmbiguityReason OnNoMatch(StepInstance stepInstance, out List<BindingMatch> matches)
        {
            /*
            ...
            var matchesWithoutScopeCheck = GetCandidatingBindings(stepInstance, useScopeMatching: false).ToList();
            ...
            var matchesWithoutParamCheck = GetCandidatingBindings(stepInstance, useParamMatching: false).ToList();
        ...
        protected IEnumerable<BindingMatch> GetCandidatingBindings(StepInstance stepInstance, bool useRegexMatching = true, bool useParamMatching = true, bool useScopeMatching = true)
        {
            var matches = bindingRegistry.GetConsideredStepDefinitions(stepInstance.StepDefinitionType, stepInstance.Text).Select(b => Match(b, stepInstance, useRegexMatching, useParamMatching, useScopeMatching)).Where(b => b.Success);
