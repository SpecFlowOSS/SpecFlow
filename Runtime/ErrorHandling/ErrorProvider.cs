...
namespace TechTalk.SpecFlow.ErrorHandling
{
    public interface IErrorProvider
    {
        ...
        Exception GetParameterCountError(StepBindingMatch match, int expectedParameterCount);
        Exception GetAmbiguousMatchError(List<StepBindingMatch> matches, StepInstance stepInstance);
        Exception GetAmbiguousBecauseParamCheckMatchError(List<StepBindingMatch> matches, StepInstance stepInstance);
        Exception GetNoMatchBecauseOfScopeFilterError(List<StepBindingMatch> matches, StepInstance stepInstance);
    ...
    internal class ErrorProvider : IErrorProvider
    {
        ...
        public Exception GetParameterCountError(StepBindingMatch match, int expectedParameterCount)
        {
            return new BindingException(
                string.Format("Parameter count mismatch! The binding method '{0}' should have {1} parameters",
                    GetMethodText(match.Binding.Method), expectedParameterCount));
        }

        public Exception GetAmbiguousMatchError(List<StepBindingMatch> matches, StepInstance stepInstance)
        {
            ...
            return new BindingException(
                string.Format("Ambiguous step definitions found for step '{0}': {1}",
                    stepDescription,
                    string.Join(", ", matches.Select(m => GetMethodText(m.Binding.Method)).ToArray())));
        }


        public Exception GetAmbiguousBecauseParamCheckMatchError(List<StepBindingMatch> matches, StepInstance stepInstance)
        {
            string stepDescription = stepFormatter.GetStepDescription(stepInstance);
            return new BindingException(
        ...
        public Exception GetNoMatchBecauseOfScopeFilterError(List<StepBindingMatch> matches, StepInstance stepInstance)
        {
            string stepDescription = stepFormatter.GetStepDescription(stepInstance);
            return new BindingException(
